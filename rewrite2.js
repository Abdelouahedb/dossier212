...
// Wrap db.run manually without regex side effects by replacing db.run( with await pool_query(
code = code.replace(/db\.run/g, 'await pool_query(db, ');

// We need to inject pool_query at the top of the file
const poolQueryFunc = `
async function pool_query(db, sql, params = []) {
  let pgSql = sql;
  let i = 1;
  while (pgSql.includes('?')) { pgSql = pgSql.replace('?', '$' + i++); }
...
console.log('Admin JS perfectly written!');
